...
class OrderServiceTest{
    ...
    void itemOrder() throws Exception{
        ...
        assertThat(8).isEqualTo(book.getStockQuantity());//주문 수량 만큼 재고가 줄어야 한다.
    }
    @Test
    @DisplayName("상품주문_재고수량초과")
    void stockQuantityOver() throws Exception{
        //given
        Member member = createMember();
        Book book = createBook("시골 JPA" , 10000 , 10);
        int orderCount  = 11;
    ...
    void orderCancel() throws Exception{
        ...
        //then
        Order getOrder = orderRepository.findOne(orderId);
        assertThat(OrderStatus.CANCEL).isEqualTo(getOrder.getStatus());//주문 취소시 상태는 CANCEL 이다.
        assertThat(10).isEqualTo(item.getStockQuantity());//주문 이 취소된 상품은 그만큼 재고가 증가해야 한다.
    }
